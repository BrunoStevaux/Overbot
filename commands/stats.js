...
module.exports.run = async (bot, message, args) => {
    const overwatchIcon = "https://upload.wikimedia.org/wikipedia/commons/thumb/5/55/Overwatch_circle_logo.svg/600px-Overwatch_circle_logo.svg.png";
    ...
    async function get_data(tag, region){
        for(const x in platforms){
            const request = await fetch(`https://ow-api.com/v1/stats/${platforms[x]}/us/${tag}/complete`, { method: 'GET' });
            if(request.status == 200) return request;
            // 400: bad username, 404: not found.
            else if (!(request.status == 400 || request.status == 404)) console.log(request.status);
        }
    ...
    else if(["playstation","psn"].includes(args[0].toLowerCase())) platform = "psn";
    ...
    const x = await get_data(battletag);
    let stats;
    ...
    // Top fields
    emb.addField('Account level', stats.prestige * 100 + stats.level, true)
    .addField('Endorsement', stats.endorsement, true);

    if(stats.private == true){
           emb.setDescription("This profile is private. To unprivate go to Carrer Profile > Social > Public.");
    } else {
        ...
        if(stats.ratings != null){
            ...
            for(let i = 0; i < stats.ratings.length; i++){
                ...
                emb.addField(`${roles[rolename]} ${roleNames[rolename]}`, getRankIcon( stats.ratings[i]['level']), true);
            }
        } else {
